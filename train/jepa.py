...
class JepaThinkModel(BaseModel):
    """具体模型实现，封装了Thinking网络"""
    ...
    def compute_loss(self, outputs, batch):
        ...
        result = {
            "loss": feal_loss,  # Note only feal_loss is included
            "feal_loss": feal_loss,
            "action_loss": action_loss,
        }
        return result
...
class Config(supervised.Config):
    model_class: type = JepaThinkModelV2
    dataset_class: type = FlappyBirdJepaDataset

    name: str = "jepa_v1"

    data_size: int = 30000
    action_data_size: int = 29000
    channel_dim: int = 4
    num_step: int = 8000
    output_dir: str = "outputs/8-19/action_data_size_29000"
